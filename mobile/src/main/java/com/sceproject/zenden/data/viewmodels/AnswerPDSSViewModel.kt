...
class AnswerPDSSViewModel : ViewModel() {
            ...
            .addSnapshotListener { snapshots, e ->
                if (e != null) {
                    ...
                    _measurementStatus.value =
                        MeasurementStatus.Connecting // or any error status you prefer
                ...
                if (snapshots != null && !snapshots.isEmpty) {
                    ...
                    _measurementStatus.value =
                        MeasurementStatus.Connecting // or any status you prefer for no data
                }
            }
    }
    ...
    fun calculateTotalPanicDisorder(heartRate: Int): Int {
        return pdssResponses.value?.values?.sum() ?: 0
    }
    ...
    fun savePdssScoreToFirestore(pdssScore: Int) {
        ...
        db.collection("users").document(userId).collection("pdss_measurements").document(documentId)
            .set(pdssMeasurement)
            .addOnSuccessListener {
                Log.d("Firestore", "PDSS Measurement successfully written!")
                // Add a delay before fetching the last measurement
